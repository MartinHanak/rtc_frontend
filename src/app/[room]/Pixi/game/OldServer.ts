...
export class Server {
    ...
    public start() {
        ...
        testWorker.postMessage(this.currentGame.getServerInitializationData())
        testWorker.onmessage = (e) => {console.log(e.data)};

        testWorker.addEventListener('error', (err) => {console.log(err.message)});
        testWorker.addEventListener('messageerror', (err) => {console.log(err)})
